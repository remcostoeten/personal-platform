  ...
  const Logo = ({ full = false, className , width, height, link, icon = false, ...props}: LogoProps) => {
    const { theme } = useTheme();
    const fill = theme === 'light' ? '#000' : '#fff';

    const svgElement = (
      <svg className={className} viewBox="0 0 115.8 46" width={`${width}px`} height={`${height}px`} {...props}>
        {icon ? (
          <>
            <path
