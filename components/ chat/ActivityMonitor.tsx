...
export default function ActivityMonitor() {
  const [mostRecentStatus, setMostRecentStatus] = useState<StatusObject | null>(
    null,
  );
  ...
  const updateStatuses = async () => {
    ...
    const mostRecent = statuses.sort(
  ...
  return (
    <>
      {isOnline ? (
        <p>
          {mostRecentStatus.name} is currently online. Has been online for{" "}
          {mostRecentStatus.timesOnline} times since {firstTimestamp}.
        </p>
      ) : (
        <p>
          {mostRecentStatus.name} is currently offline. Last seen at{" "}
          {mostRecentStatus.lastSeen}. The last online session lasted{" "}
          {mostRecentStatus.lastOnlineDuration}.
        </p>
      )}
    </>
  );
